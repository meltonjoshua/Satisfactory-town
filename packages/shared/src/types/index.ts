...
export interface FluidFlowEntry {
  itemId: string;
  itemName: string;
  rate: number;
  source: string;
...
export interface FluidGroupBalance {
  groupId: string;
  itemId: string;
  itemName: string;
  inputRate: number;
  ...
  overflowRate: number;
  status: 'balanced' | 'overflow' | 'deficit' | 'dead-end';
  connections: FluidFlowEntry[];
}

export interface FluidBalance {
  groups: FluidGroupBalance[];
  totalPipes: number;
